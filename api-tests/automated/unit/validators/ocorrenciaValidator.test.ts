import { z } from 'zod';
import { createOcorrenciaSchema, listOcorrenciaSchema } from '../../../../src/validators/ocorrenciaValidator';

// UUIDs válidos e genéricos para evitar erros de validação Zod
...
describe('Ocorrencia Validators', () => {
  ...
  describe('createOcorrenciaSchema', () => {
    ...
    it('deve aceitar payload com todos os dados e UUIDs válidos', async () => {
      const result = await createOcorrenciaSchema.parseAsync({ body: validData });
    ...
    it('deve falhar se id_subgrupo_fk não for um UUID', async () => {
      ...
      await expect(createOcorrenciaSchema.parseAsync({ body: invalid })).rejects.toThrow(z.ZodError);
    });
  ...
  describe('listOcorrenciaSchema', () => {
    ...
    it('deve aceitar filtros de status, data e paginação corretos', async () => {
      ...
      // Garante que os números vieram como number
    ...
    it('deve falhar se o status for inválido', async () => {
      ...
      await expect(listOcorrenciaSchema.parseAsync({ query })).rejects.toThrow(z.ZodError);
    });
