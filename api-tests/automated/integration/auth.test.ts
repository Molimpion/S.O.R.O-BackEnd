// api-tests/automated/integration/auth.test.ts

import request from 'supertest';
import app from '../../../src/index';

// 1. MOCKANDO O SERVIÇO DE AUTENTICAÇÃO INTEIRO
...
describe('Auth Routes (Integration Tests)', () => {
  ...
  const mockUser = { id: 'user-id-mock', email: 'test@test.com', nome: 'Test User' };
  const mockToken = 'mock-jwt-token-valido';

  // --- LOGIN TESTS ---
  it('POST /api/v1/auth/login - Sucesso (Status 200)', async () => {
    // Simula sucesso no serviço: retorna user e token
  ...
  it('POST /api/v1/auth/login - Falha (Credenciais Inválidas) (Status 401)', async () => {
    // Simula falha no serviço: lança UnauthorizedError
    (authService.loginUser as jest.Mock).mockRejectedValue(new UnauthorizedError('Email ou senha inválidos'));
  ...
  // --- REGISTER TESTS ---
  const validRegisterData = { name: 'Novo User', email: 'new@user.com', profile: 'ANALISTA', matricula: 'M123' };
  
  it('POST /api/v1/auth/register - Sucesso (Status 201)', async () => {
    // Simula sucesso no serviço: retorna o novo user
  ...
  it('POST /api/v1/auth/register - Falha (Erro de Serviço/E-mail) (Status 500)', async () => {
    // Simula falha no serviço: lança ApiError (simulando falha de e-mail com rollback)
    (authService.registerUser as jest.Mock).mockRejectedValue(
        new ApiError('Não foi possível criar o usuário: falha ao enviar o e-mail de confirmação.', 500)
    );
