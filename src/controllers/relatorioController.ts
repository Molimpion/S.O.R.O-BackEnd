...
export const exportRelatorio = async (req: Request, res: Response) => {
  ...
  if (type === "csv") {
    const csvString = await ocorrenciaService.exportOcorrenciasToCSV(
      ocorrencias
    );
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=relatorio_ocorrencias.csv"
    );

    return res.status(200).send(csvString);
  } else if (type === "pdf") {
    const pdfBuffer = await ocorrenciaService.exportOcorrenciasToPDF(
      ocorrencias
    );
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=relatorio_ocorrencias.pdf"
    );
