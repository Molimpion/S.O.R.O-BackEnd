  ...
  try {
    ...
    if (
      ...
      throw new ConflictError(
        "Já existe uma Unidade Operacional com este nome."
      );
    }
    throw error;
  }
...
export async function deleteUnidade(id: string) {
  ...
  const usuarioEmUso = await prisma.user.findFirst({
    where: { id_unidade_operacional_fk: id },
  });
  if (usuarioEmUso) {
    throw new ConflictError(
      "Esta unidade não pode ser deletada pois possui usuários associados."
    );
  }

  const viaturaEmUso = await prisma.viatura.findFirst({
    where: { id_unidade_operacional_fk: id },
  });
  if (viaturaEmUso) {
    throw new ConflictError(
      "Esta unidade não pode ser deletada pois possui viaturas associadas."
    );
  }
